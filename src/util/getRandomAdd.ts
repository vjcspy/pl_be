...
export class CheckoutCache {
  static _addData = null;
}

export const getCacheAdd = (saveToCache?: any) => {
  if (typeof saveToCache !== 'undefined') {
    CheckoutCache._addData = saveToCache;
  }

  return CheckoutCache._addData;
};
let loading = false;
export const getRandomAdd = async () => {
  ...
  try {
    ...
    loading = true;
    console.log('generate new random add');
    ...
    loading = false;
    console.log('done generate random address');
    return {
      addDataArr,
      addNameArr,
    };
